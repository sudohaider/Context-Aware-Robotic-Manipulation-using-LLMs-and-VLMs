  ...
  : node_{ std::make_shared<rclcpp::Node>("mtc_node", options) }
{
    ...
    this->task_completed_pub_ = node_->create_publisher<std_msgs::msg::Bool>(
        "task_completed",
        10);
}
...
void MTCTaskNode::execute(const std::shared_ptr<GoalHandleFibonacci> goal_handle)
{
    ...
    // execute the action:
    param_value = action_name;
    doTask();

    loop_rate.sleep();

    // execute the action/call the action function here:
    RCLCPP_INFO(LOGGER, "performing action: %s", goal->action_name.c_str());
    ...
    if (rclcpp::ok())
    {
        ...
        RCLCPP_INFO(LOGGER, "action %s complete", goal->action_name.c_str());
        
        
        //publish true" on /task_completed topic:
        std_msgs::msg::Bool task_completed_msg;
        task_completed_msg.data = true;
        this->task_completed_pub_->publish(task_completed_msg);
    }
}

void MTCTaskNode::handle_accepted(const std::shared_ptr<GoalHandleFibonacci> goal_handle)
{
    using namespace std::placeholders;
...
void MTCTaskNode::setupPlanningScene()
{
    ...
    pose.position.x = 0.5; // 0.5;
    pose.position.y = -0.25; //-0.25;
    pose.position.z = 0.3; 
    ...
    psi2.applyCollisionObject(object);
}
...
void MTCTaskNode::doTask()
{
    ...
    RCLCPP_INFO(LOGGER, "reached execution");

    auto result = task_.execute(*task_.solutions().front());
    RCLCPP_INFO(LOGGER, "got result");
    ...
    RCLCPP_INFO(LOGGER, "Task executed successfully: %d", result.val);
    return;
}
...
#pragma GCC diagnostic pop
    ...
    else if (param_value == "move to place")
    {
        auto stage_open_hand = std::make_unique<mtc::stages::MoveTo>("open hand", interpolation_planner);
        stage_open_hand->setGroup(hand_group_name);
        stage_open_hand->setGoal("open");
...

        {
...

            {
                ...
                target_pose_msg.pose.position.y = 0.25; // 0.5
    ...
    else if (param_value == "ungrasp")
    {
        auto ungrasp = std::make_unique<mtc::SerialContainer>("ungrasp");
        {
            auto stage = std::make_unique<mtc::stages::MoveTo>("open hand", interpolation_planner);
            stage->setGroup(hand_group_name);
            stage->setGoal("open");
            ungrasp->insert(std::move(stage));
        }
