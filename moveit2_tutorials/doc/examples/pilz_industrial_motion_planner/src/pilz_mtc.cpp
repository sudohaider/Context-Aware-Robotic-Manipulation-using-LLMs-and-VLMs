...
void MTCTaskNode::doTask()
{
  ...
  // After task_.plan() is called
if (task_.plan()) {
    RCLCPP_INFO(LOGGER, "Task planned successfully!");

    // Iterate over all stages in the task
    for (const auto &stage : task_.stages()) {
        // Check if the stage has solutions
        if (stage->solutions().empty()) {
            RCLCPP_WARN(LOGGER, "Stage '%s' has no solutions", stage->name().c_str());
            continue;
        }

        // Access the first solution of the stage
        const auto &solution = stage->solutions().front();

        // Access the robot state after the planner
        const auto &robot_state = solution->end()->scene()->getCurrentState();

        // Extract joint positions
        std::vector<double> joint_positions;
        robot_state.copyJointGroupPositions("arm_group_name", joint_positions);  // Replace with your joint group name

        // Print the joint positions
        RCLCPP_INFO(LOGGER, "Joint positions after stage '%s':", stage->name().c_str());
